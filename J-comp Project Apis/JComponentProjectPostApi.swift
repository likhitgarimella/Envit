...
class JComponentProjectPostApi {
    ...
    var REF_POSTS = Database.database().reference().child("J-Component Projects").child("Details")
    ...
    func observePost(withId id: String, completion: @escaping (JComponentProjectModel) -> Void) {
    ...
    func incrementLikes(postId: String, onSuccess: @escaping (JComponentProjectModel) -> Void, onError: @escaping (_ errorMessage: String?) -> Void ) {
        
        let postRef = Api.JComponentProjectPost.REF_POSTS.child(postId)
